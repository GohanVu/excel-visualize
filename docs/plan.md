...
| P2-T3 | Free tier gate: giới hạn 3 chart / dashboard | ✅ Done | P2-T2 | BE gate trong `saveChart` (count chart/dashboard, Free=3, Pro ∞); FE hiện nudge nâng cấp từ message backend. Phần "min(số chart hợp lý từ data)" → T4 (locked slot UI) |
| P2-T4 | UI locked chart slot: blur + 🔒 + nudge nâng cấp | ✅ Done | P2-T3 | `GET /charts` trả `limit` (Free=3/Pro=null). DashboardPage hiện LockedChartSlot khi Free đạt cap (charts≥limit); dưới cap/Pro → ẩn. CTA nâng cấp defer (billing Phase 5) |
...
| P3.5-T4 | Lưu config v2 `{version:2, definition, option}` khi save; render đọc CẢ v2 + config cũ | ✅ Done | P3.5-T3 | Type `ChartConfigV2` + `toChartConfigV2`/`resolveChartOption`. ChartDetailPage lưu `{version,definition,option}` (cache option để Dashboard render khỏi fetch rows). Dashboard grid + ChartStylePanel dùng `resolveChartOption`; ChartStylePanel v2-aware (sửa option cache, giữ definition). Chart cũ (option thô) vẫn render + sửa style OK. Backend **KHÔNG đổi schema**. +6 test |
